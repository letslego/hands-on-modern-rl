...
\node[box] (env) at (5.5, 0) {\\[-2pt]{\small (Environment)}\\[-4pt]{\footnotesize //API}};

\draw[arrow] ([yshift=4pt]agent.north east) -- node[above, lbl, pos=0.5] { $a_t$} ([yshift=4pt]env.north west);

\draw[arrow] ([yshift=-4pt]env.south west) -- node[below, lbl, pos=0.5] { $o_{t+1}$} ([yshift=-4pt]agent.south east);
