...
#### 1. M2.1 的 SWE Scaling：从 GitHub 原始数据到可运行 RL 环境。
...
#### 2. M2.1 的 AppDev：从固定测试转向 Agent-as-a-Verifier。
...
#### 3. WebExplorer：解决长程网页 agent 缺少难题数据的问题。
...
#### 1. Qwen2.5：先把通用 instruct 做成多阶段后训练。
...
#### 2. Qwen2.5-Math：数学后训练的核心是 CoT / TIR 自改进闭环。
...
#### 3. QwQ-32B： outcome-based reward 的两阶段 RL。
...
#### 4. Qwen3：thinking / non-thinking 的统一后训练。
...
#### 5. Qwen3-Coder / Tongyi DeepResearch：从答案 RL 进入过程 RL。
...
#### 1. Kimi k1.5：不用 MCTS / value function / process RM，也能做长思考 RL。
...
#### 2. 长度奖励：解决 overthinking，而不是简单砍 max tokens。
...
#### 4. Kimi K2：agentic intelligence 的数据和工具闭环。
...
#### 5. Kimi-Researcher：研究 agent 的 reward 要覆盖证据链。
...
#### 1. Seed1.5-Thinking：reasoning model 的基础配方。
...
#### 3. VAPO：value model 不是不能用，关键是长 CoT 的 advantage 要重做。
...
#### 4. UI-TARS：GUI agent 先靠轨迹和偏好学基本操作。
...
#### 6. Seed Prover 1.5：形式化证明环境里的 agentic RL。
...
#### 2. DeepSeek-R1-Zero：直接从 base model 做规则 RL。
...
#### 4. DeepSeek-V3.2：从答案 verifier 到 agentic verifier。
...
#### 1. GLM-4.5：hybrid reasoning 和 expert model iteration。
...
#### 3. 异步 agent RL：把 generation 和 training 解耦。
...
#### 4. On-policy cross-stage distillation：防止分阶段训练遗忘。
...
#### 1. Hunyuan-T1：把大部分 post-training compute 投给 RL。
...
#### 4. Hunyuan-A13B：fast / slow thinking 的产品形态。
...
#### 2. Unified Rewarding System：把 rule、sandbox、RDRM、GRM 等奖励放进同一框架。
...
#### 1. STEP3-VL-10B：fully unfrozen 预训练后，用 1K+ RL iteration 做视觉推理。
...
#### 4. Step-DeepResearch：训练研究过程而不是报告模板。
...
#### 3. 最小复现：给偏好数据加 domain / style / safety 条件。
...
#### 1. Baichuan2：中文开源语境下的 SFT -> RM -> PPO。
...
#### 1. Skywork-OR1：在 R1-Distill 上继续 RL，核心风险是 entropy collapse。
...
#### 2. MiMo：7B reasoning model 的 post-training 关键是 130K 可验证题。
...
#### 2. GPT-4 到 o-series：reasoning 后训练把动作空间扩大。
...
#### 3. Deliberative alignment：安全也变成推理任务。
...
#### 4. Operator / Codex：agent 后训练需要真实环境。
...
#### 1. Constitutional AI：把安全偏好写成原则，再用 AI feedback 扩展数据。
...
#### 1. Gemini 1.5 / 2.5：长上下文后训练的核心是 evidence grounding。
...
#### 2. Deep Think：横向探索 + 汇总，而不是无限延长单条 CoT。
...
#### 4. Gemma：开放小模型的 distillation + targeted post-training。
...
#### 2. Rejection sampling：用 RM / 规则把采样变成新 SFT 数据。
...
#### 1. 数据先行：teachable prompts 比大而杂更重要。
...
#### 2. Llama Nemotron：prune/distill 后做 reasoning 和 agent 后训练。
...
#### 2. Remote reward functions：企业 verifier 接进 RL。
...
#### 3. 最小复现：本地 verifier 模拟企业 reward。
...
#### 1. Decentralized pipeline：避免所有能力串行互相覆盖。
...
#### 3. 最小复现：3-6 个 expert track + soup + polish。
...
#### 1. Databricks DBRX：企业开源 instruct 的基础线。
...
#### 3. Cursor Composer 2：coding agent 训练要用真实仓库任务。
